...
=item In B<untyped> mode, the list can hold values of any type and also of
different type. One creates such a list using the C<List_create> constructor.

=item In B<typed> mode, the list is fixed to one type. Only elements of that
...
=item The two lists have the same type constraint.

=item Each element is C<Variants.equals>. This means that objects are compared
using C<Is>.
...
L<https://github.com/HALTEC/vba-library/issues>.
